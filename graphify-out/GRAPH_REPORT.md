...
## Corpus Check
- 46 files · ~13,142 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 310 nodes · 546 edges · 19 communities (13 shown, 6 thin omitted)
...
Cohesion: 0.06
Nodes (12): Db, MemoryDb, nowIso(), SupabaseDb, uid(), AgentSuggestion, KnowledgeEdge, KnowledgeItem (+4 more)
...
Cohesion: 0.06
Nodes (31): dependencies, animejs, @anthropic-ai/sdk, framer-motion, gsap, lenis, lucide-react, next (+23 more)
...
### Community 4 - "db.ts"
Cohesion: 0.12
Nodes (18): GET(), DELETE(), GET(), PATCH(), POST(), POST(), DELETE(), GET() (+10 more)
...
Cohesion: 0.13
Nodes (10): fraunces, geistMono, geistSans, metadata, MAIN, MEDIA, MobileNav(), NavItem (+2 more)
...
Cohesion: 0.08
Nodes (30): HubPlatformCard(), Props, MetricCard(), Props, MetricGroup, MetricsSidebar(), Props, OverallCard() (+22 more)
...
- **Why does `MemoryDb` connect `MemoryDb` to `types.ts`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `SupabaseDb` connect `MemoryDb` to `types.ts`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `getDb()` connect `db.ts` to `ingest.ts`, `types.ts`, `route.ts`, `MemoryDb`?**
